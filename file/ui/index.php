    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if ($_GET['act'] == 'add') {
            if ($_POST['iprange0'] && $_POST['dspeed'] && $_POST['uspeed']) {
                ...
                $mulid = dechex(time());
                $uname = 'PolBan_u' . $mulid;
                $dname = 'PolBan_d' . $mulid;
            ...
            if (strpos($custom_firewall, '/root/.PolBan/run') !== FALSE) {
                shell_exec("echo \"" . trim(str_replace('/root/.PolBan/run', '', $custom_firewall), "\n") . "\n" . "\" > /etc/firewall.user");
            }
            $list = shell_exec('iptables -S');
            $list = str_replace("\r\n", "\n", $list);
            $list = explode("\n", $list);
...
?>
    ...
    <body style="background-color: #dedede;">
        <div class="wraper py-4 bg-white px-3">
            <h1 class="text-center">PolBan</h1>
            ...
            <hr class="mb-4">
            <div id="home-page">
                <form method="post" id="PolBanFormAdd">
                            ...
                            if (strpos($ls, 'PolBan_d') !== FALSE) {
                                ...
                                foreach ($rule as $i => $rl) {
                                    ...
                                    if ($rl == '--hashlimit-name') {
                                        $mulid = str_replace('PolBan_d', 'PolBan_u', $rule[$i + 1]);
                                    }
                                }
        ...
        <script>
            ...
            $("#PolBanFormAdd").on('submit', function(e) {
                e.preventDefault();
            ...
            $("#PolBanFormAdd").on('click', '#btnCancelEdit', function() {
                state.formEditType = 'add'
                $('#PolBanFormAdd').find('[type=submit]').val('Add')
                $("#PolBanFormAdd")[0].reset()
                $(this).remove()
            })

            $("#PolBanFormPassword").on('submit', function(e) {
                e.preventDefault();
            ...
            function editRule(el) {
                ...
                $('#PolBanFormAdd').find('[name=iprange0]').val(iprange0).focus()
                $('#PolBanFormAdd').find('[name=iprange1]').val(iprange1)
                $('#PolBanFormAdd').find('[name=dspeed]').val(dspeed)
                $('#PolBanFormAdd').find('[name=uspeed]').val(uspeed)
                $('#PolBanFormAdd').find('[name=timestart]').val(timestart)
                $('#PolBanFormAdd').find('[name=timestop]').val(timestop)

                $('#PolBanFormAdd').find('[name^=weekdays]').prop('checked', false)
                weekdays.forEach(v => {
                    $('#PolBanFormAdd').find('[name^=weekdays][value=' + v + ']').prop('checked', true)
                })

                if (!$('#PolBanFormAdd').find('#btnCancelEdit').length) {
                    $('#PolBanFormAdd').find('[type=submit]').val('Save').after(`<input id="btnCancelEdit" type="reset" class="btn btn-danger btn-sm ms-1" value="Cancel">`)
                }
            ...
            function uninstallMe(el) {
                if (confirm("Do you want to uninstall PolBan?")) {
...
?>
    ...
    <body style="background-color: #dedede;">
        ...
        <script>
            ...
            $("#PolBanFormLogin").on('submit', function(e) {
                e.preventDefault();
